        ...
        <section className="min-w-0 space-y-5">
          ...
          <section className="min-w-0 rounded-lg border border-[#dbe5d8] bg-white">
            <div className="flex items-center justify-between border-b border-[#e4ece1] p-4">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => downloadCsv("LCHF-personal-food-items.csv", [["Item", "Owner Email", "Carbohydrates", "Proteins", "Fats", "Calories"], ...filteredPersonalFoodItems.map((item) => [item.itemName, item.ownerEmail, item.carbohydrates, item.proteins, item.fats, item.calories])])} className="rounded-md border border-[#d8e2d5] p-2 hover:bg-[#f4f7f2]" aria-label="Download personal food items"><Download className="size-4" /></button>
            ...
            <div className="hidden overflow-x-auto lg:block">
              <table className="w-full min-w-[760px] text-sm">
                ...
                <tbody>{filteredPersonalFoodItems.length === 0 ? <tr><td colSpan={canManagePersonal ? 7 : 6} className="p-4 text-center text-[#6a7669]">No personal food items found.</td></tr> : filteredPersonalFoodItems.map((item) => <tr key={item.id} className="border-t border-[#eef3ec]"><td className="p-3 font-medium">{item.itemName}</td><td className="p-3">{round(item.carbohydrates)}g</td><td className="p-3">{round(item.proteins)}g</td><td className="p-3">{round(item.fats)}g</td><td className="p-3">{round(item.calories, 0)}</td>{canManagePersonal ? <td className="p-3"><div className="flex gap-2"><button type="button" onClick={() => openEditPersonalModal(item)} className="rounded-md border border-[#d8e2d5] p-2 hover:bg-[#f4f7f2]"><Pencil className="size-4" /></button><form action={deletePersonalFoodItem}><input type="hidden" name="id" value={item.id} /><button className="rounded-md border border-[#ead0cb] p-2 text-[#a13f32] hover:bg-[#fff4f2]"><Trash2 className="size-4" /></button></form></div></td> : null}</tr>)}</tbody>
              </table>
            </div>
